...
class QueueSizeTester:
    ...
    def test_single_queue_size(self, queue_size):
        """단일 큐 크기 테스트"""
        print(f"\n{'='*70}")
        print(f"[테스트] 큐 크기 {queue_size} 테스트 시작")
        print(f"{'='*70}")
        
        # 큐 초기화
        inference_queue = queue.Queue(maxsize=queue_size)
        result_queue = queue.Queue(maxsize=max(4, queue_size // 2))
        running = True
        ...
        while running:
            ...
            # 적절한 대기 제거 - 최대 속도로 전송 (GPU 성능 측정)
            # time.sleep(1.0 / (video_fps * 2))  # 주석 처리
        ...
        avg_inference_time = total_inference_time / (frames_processed / self.batch_size) if frames_processed > 0 else 0
    ...
    def run_optimization_test(self):
        """큐 크기 최적화 테스트 실행"""
        print(f"\n{'='*70}")
        print("[테스트] 큐 크기 최적화 테스트 시작")
        print("[테스트] FPS가 5회 연속 감소하면 테스트 종료")
        print(f"{'='*70}\n")
        
        results = []
        consecutive_decreases = 0
        max_queue_size = 50  # 최대 테스트 크기 (100 → 50, 빠른 테스트)
        
        for queue_size in range(1, max_queue_size + 1):
            fps = self.test_single_queue_size(queue_size)
            results.append((queue_size, fps))
        ...
        print("[테스트] 최종 결과")
        print(f"{'='*70}")
        print(f"{'큐 크기':<10} | {'FPS':<10} | {'성능 변화':<15}")
        print(f"{'-'*40}")
        ...
        for i, (queue_size, fps) in enumerate(results):
            ...
            if fps > max_fps:
                max_fps = fps
                optimal_queue_size = queue_size
                marker = " ⭐ 최고"
            
            print(f"{queue_size:<10} | {fps:<10.2f} | {change:<15} {marker}")
        
        print(f"\n{'='*70}")
        print(f"[테스트] 🏆 최적 큐 크기: {optimal_queue_size} (FPS: {max_fps:.2f})")
        print(f"{'='*70}\n")
...
self.inference_queue = queue.Queue(maxsize={optimal_queue_size})
self.result_queue = queue.Queue(maxsize={max(4, optimal_queue_size // 2)})
        """)
        
        return optimal_queue_size, max_fps, results
    
    def visualize_results(self, results):
        """결과 시각화"""
        print(f"\n[테스트] 결과 시각화 중...")
        
        queue_sizes = [r[0] for r in results]
        fps_values = [r[1] for r in results]
        
        # 최적값 찾기
        max_fps = max(fps_values)
        optimal_idx = fps_values.index(max_fps)
        optimal_queue_size = queue_sizes[optimal_idx]
        
        # Figure 생성 (3개 서브플롯)
        fig, axes = plt.subplots(3, 1, figsize=(14, 12))
        fig.suptitle('큐 크기 최적화 테스트 결과', fontsize=16, fontweight='bold')
        
        # 1. FPS vs 큐 크기 (선 그래프)
        ax1 = axes[0]
        ax1.plot(queue_sizes, fps_values, 'b-', linewidth=2, label='FPS')
        ax1.plot(optimal_queue_size, max_fps, 'r*', markersize=20, label=f'최적값 (큐={optimal_queue_size}, FPS={max_fps:.2f})')
        ax1.axhline(y=max_fps, color='r', linestyle='--', alpha=0.3)
        ax1.axvline(x=optimal_queue_size, color='r', linestyle='--', alpha=0.3)
        ax1.set_xlabel('큐 크기', fontsize=12)
        ax1.set_ylabel('FPS', fontsize=12)
        ax1.set_title('FPS vs 큐 크기', fontsize=14, fontweight='bold')
        ...
        ax2.bar(queue_sizes, changes, color=colors, alpha=0.6)
        ax2.axhline(y=0, color='black', linestyle='-', linewidth=0.5)
        ax2.set_xlabel('큐 크기', fontsize=12)
        ax2.set_ylabel('성능 변화율 (%)', fontsize=12)
        ax2.set_title('이전 큐 크기 대비 성능 변화율', fontsize=14, fontweight='bold')
        ...
        ax3.fill_between(queue_sizes, 0, cumulative_improvement, alpha=0.3, color='blue')
        ax3.plot(queue_sizes, cumulative_improvement, 'b-', linewidth=2)
        ax3.plot(optimal_queue_size, cumulative_improvement[optimal_idx], 'r*', markersize=20)
        ax3.axhline(y=0, color='black', linestyle='-', linewidth=0.5)
        ax3.axhline(y=cumulative_improvement[optimal_idx], color='r', linestyle='--', alpha=0.3)
        ax3.set_xlabel('큐 크기', fontsize=12)
        ax3.set_ylabel('누적 성능 개선 (%)', fontsize=12)
        ax3.set_title(f'큐 크기 1 대비 누적 성능 개선 (최대: {cumulative_improvement[optimal_idx]:.1f}%)', 
        ...
        # 저장
        output_path = os.path.join(current_dir, 'queue_size_optimization_result.png')
...
def main():
    ...
    # 테스터 생성 및 실행
    tester = QueueSizeTester(video_path, device=device)
    optimal_size, max_fps, results = tester.run_optimization_test()
    
    print(f"\n[테스트] 완료! 최적 큐 크기: {optimal_size}, 최대 FPS: {max_fps:.2f}")
